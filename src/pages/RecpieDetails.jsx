...
const RecpieDetails = () => {
  ...
  const getRecipe = async () => {
    try{
      ...
      const reccomend = await fetchRecipes({query: recipe?.label, limit:5})

      setRecipes(reccomend)
      setLoading(false)
    }
    catch(error)
    {
      ...
      <div className='w-full px-4 lg:px-20 pt-5' >
        <div className='flex gap-10 items-center justify-center px-4' >
          ...
          <div className='flex flex-col justify-center'>
            ...
            <p className='text-neutral-100 text-[10px] md:text-md'>SERVINGS</p>
          </div>
        </div>
        ...
        <div className='w-full flex flex-col md:flex-row gap-8 py-20 px-4 md:px-10'>
          ...
          <div className='w-full md:w-2/4 md:border-r border-slate-800 pr-1'>
            <div className='flex flex-col gap-5'>
              <p className='text-green-500 text-2xl'>Ingredients</p>
              {
